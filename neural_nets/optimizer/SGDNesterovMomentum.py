...
class SGDNesterovMomentumParamsInitVisitor(Visitor):
    ...
    def visit_conv2d(self, layer: Conv2DTrain):
        ...
        layer_velocity.add(name=Name.V_KERNEL_WEIGHTS_PREV, value=np.zeros_like(weights.get(name=Name.KERNEL_WEIGHTS)))

        layer_velocity.add(name=Name.V_KERNEL_BIASES, value=np.zeros_like(weights.get(name=Name.KERNEL_BIASES)))
        layer_velocity.add(name=Name.V_KERNEL_BIASES_PREV, value=np.zeros_like(weights.get(name=Name.KERNEL_BIASES)))
...
class SGDNesterovMomentumWeightsUpdateVisitor(Visitor):
    ...
    def visit_conv2d(self, layer: Conv2DTrain):
        ...
        v_prev_names = [Name.V_KERNEL_WEIGHTS_PREV, Name.V_KERNEL_BIASES_PREV]
        self._update(weight_names=weight_names, grad_names=grad_names, v_names=v_names, v_prev_names=v_prev_names,
                     layer=layer)
